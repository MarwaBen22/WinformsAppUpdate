...
{
    public partial class UC_DashboardFicheRenseign : UserControl
    {
        private dashboardClass model;
        public UC_DashboardFicheRenseign()
        {
            InitializeComponent();
            DisableCustomDates();
        ...
        {
            var refreshData = model.LoadDataFRens(dtpStartDate.Value, dtpEndDate.Value);
            if (refreshData == true)
            {
                chartFRensEtat.DataSource = model.FiRensEtatList;
                chartFRensEtat.Series[0].XValueMember = "Key";
                chartFRensEtat.Series[0].YValueMembers = "Value";
                chartFRensEtat.DataBind();

                chartTurnOver.DataSource = model.TurnOverList;
                chartTurnOver.Series[0].XValueMember = "Key";
                chartTurnOver.Series[0].YValueMembers = "Value";
                chartTurnOver.DataBind();
        ...
        void PrintChart(object sender, PrintPageEventArgs ev)
        {
            ...
            {
                chartFRensEtat.Printing.PrintPaint(ev.Graphics, marginBounds);
            }
            else if (combCharts.SelectedIndex == 1)
            {
                chartTurnOver.Printing.PrintPaint(ev.Graphics, marginBounds);
            }
        }
        ...
        private void UC_DashboardFicheRenseign_Load(object sender, EventArgs e)
        {
