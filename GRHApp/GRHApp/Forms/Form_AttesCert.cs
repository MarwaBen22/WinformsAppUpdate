...
{
    public partial class Form_AttesCert : Form
    {
        public Form_AttesCert()
        {
            InitializeComponent();
        }
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    if(crud.check_Id("T_AttesCert", "num_Ref", txtNumber.Text) == "Code exists")
                    {
                    ...
                    {
                        crud.crudop("insert into T_AttesCert (num_Ref, id_Emp, c.créé_le, c.créé_par) values ('" + txtNumber.Text + "','" + txtSearch.Text + "',CONVERT (datetime, '" + DateTime.Now + "', 121),'" + user + "')");
                        
                        crud.crudop("insert into T_Journal values (((select max(id_Journal) from T_Journal) + 1), 'Ajout de nouvelle attestation','" + user + "',CONVERT (datetime, '" + DateTime.Now + "', 121))");
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            e.Graphics.DrawString(" ATTESTATION DE TRAVAIL ", new Font("Arial", 22, FontStyle.Bold), Brushes.Black, new Point(200, 340));
            ...
            e.Graphics.DrawString(" Est Employé au sein de notre entreprise du: ", new Font("Arial", 13, FontStyle.Regular), Brushes.Black, new Point(70, 570));
            e.Graphics.DrawString("  " + dtpRecruit.Value.ToShortDateString() + " à ce jour.", new Font("Arial", 13, FontStyle.Bold), Brushes.Black, new Point(410, 570));

            e.Graphics.DrawString(" Le poste actuel qu'il occupe est: ", new Font("Arial", 13, FontStyle.Regular), Brushes.Black, new Point(70, 600));
            e.Graphics.DrawString("  " + txtJob.Text + " .", new Font("Arial", 13, FontStyle.Bold), Brushes.Black, new Point(325, 600));

            e.Graphics.DrawString(" La présente attestaion est délivrée pour servir et valoir ce que de droit. ", new Font("Arial", 13, FontStyle.Regular), Brushes.Black, new Point(70, 650));
            ...
            crud.crudop("insert into T_Journal values (((select max(id_Journal) from T_Journal) + 1), 'Impression d attestation','" + user + "',CONVERT (datetime, '" + DateTime.Now + "', 121))");
